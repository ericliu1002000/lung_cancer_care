...
class FollowupReviewTemplateTests(TestCase):
    def test_followup_config_modal_has_catalog_controls_and_htmx_hooks(self):
        ...
        self.assertIn("window.Alpine.$data(root).openConfig()", html)
        ...
        self.assertIn('@click.self="closeConfig()"', html)
        self.assertIn("@click.stop", html)
        self.assertIn('hx-include="#routine-filter-form,#followup-review-config-form"', html)
        self.assertNotIn('@click.outside="closeConfig()"', html)
        self.assertIn('@htmx:before-request="configOpen = false"', html)
        self.assertIn('@htmx:after-request="configOpen = false"', html)
        self.assertIn("window.Alpine.initTree(section)", html)
