...
def questionnaire_detail(request, patient_id):
    ...
    # 1.获取当前患者的所有疗程记录，并且筛选出当前疗程下的已记录随访问卷并完成的日期，
    # 过滤掉没有勾选问卷的日期、以及已勾选的日期但是用户当天没填写，数据按照日期降序排列
    # 2.用户默认选择最近的日期，去查询当天的随访问卷模块数据，页面显示的内容包括：
    #问卷名称、当前评分、较上次对比提升/下降分数、题目的列表数据，包含：题目、本次回答(回答的日期)、上次回答（上次回答的日期）、变化（和上次回答对比，分数是提升/下降多少分）
